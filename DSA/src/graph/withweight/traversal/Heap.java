...
public class Heap extends NodeCollection {
    ...
    public void swap(String n1, String n2) {
        ...
        nodes.set(p1, n2);
        //System.out.println("swap: " + p1 + " " + n2 +"swap");
        indices.put(n2, p1);
        //System.out.println("swap: " + n2 + " " + p1 );
        nodes.set(p2, n1);
        // System.out.println("swap: " + p2 + " " + n1 + "swap");
        indices.put(n1, p2);
    ...
    public void moveDown(String n) {
        ...
        Optional<String> minChild = getMinChild(n);


        if (minChild.isPresent()) {
            String minChildNode = minChild.get();
            if (weights.get(n).compareTo(weights.get(minChildNode)) > 0) {
                swap(n, minChildNode);
                moveDown(n);
            }
        }
    ...
    public void moveUp(String n) {
        ...
        if (parent.isPresent()) {
            String parentNode = parent.get();
            if (weights.get(n).compareTo(weights.get(parentNode)) < 0) {
                swap(n, parentNode);
                moveUp(n);
            }
        }
